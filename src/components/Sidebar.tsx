...
const menuGroups = [
  {
    ...
    menuItems: [
      {
        icon: (<IoHomeOutline size={25} color="current" />),
        label: 'Dashboard',
        route: '/dashboard',
      },
      {
        icon: (<IoCreate size={25} color="current" />),
        label: 'Create task',
        route: '/create-task'
      }
    ]
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      ...
      >
        {/* <!-- SIDEBAR HEADER --> */}
        <div className="flex items-center justify-between gap-2 px-6 py-1 lg:py-2 xl:py-3">
          <button
